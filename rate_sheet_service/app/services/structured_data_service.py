...
class StructuredDataService:
            ...
            if pricing_tiers:
                for tier in pricing_tiers:
                    ...
                    record = Route(
                        ...
                        space_available=self._safe_int(route_data.get("space_available")),
                        space_unit=route_data.get("space_unit") or (None if not route_data.get("space_available") else "TEU"),
                        valid_from=valid_from,
                        valid_to=valid_to,
                        extra_data={
    ...
    async def get_structured_data_full(
        self,
        session: AsyncSession,
        rate_sheet_id: str,
        organization_id: int
    ) -> Optional[Dict[str, Any]]:
        """Get full structured data for a rate sheet (metadata + routes + pricing_tiers + surcharges) as JSON-serializable dict."""
        try:
            result = await session.execute(
                select(RateSheetStructuredData)
                .options(
                    selectinload(RateSheetStructuredData.routes),
                    selectinload(RateSheetStructuredData.pricing_tiers),
                    selectinload(RateSheetStructuredData.surcharges),
                )
                .where(
                    and_(
                        RateSheetStructuredData.rate_sheet_id == rate_sheet_id,
            ...
            for s in record.surcharges:
                surcharges.append({
                    "id": s.id,
                    "surcharge_code": s.surcharge_code,
                    ...
                    "valid_from": _dt(s.valid_from),
                    "valid_to": _dt(s.valid_to),
            ...
            return {
                ...
                "rate_sheet_type": record.rate_sheet_type,
                "title": record.title,
                "status": record.status,
                "valid_from": _dt(record.valid_from),
                "valid_to": _dt(record.valid_to),
                "effective_date": _dt(record.effective_date),
            ...
            logger.error(f"Error getting full structured data for {rate_sheet_id}: {e}", exc_info=True)
            return None
            ...
            for route, rate_sheet in rows:
                matching_routes.append({
                    ...
                    "route": {
                        ...
                        "space_available": route.space_available,
                        "space_unit": route.space_unit,
