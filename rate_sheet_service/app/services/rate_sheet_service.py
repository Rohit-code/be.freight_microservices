...
class RateSheetService:
    ...
    @staticmethod
    def _parse_origin_destination_from_query(query: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
        """Parse origin and destination port names from a natural language query. Returns (origin, destination) or (None, None)."""
        if not query or not query.strip():
            return None, None
        q = query.strip()
        # "from X to Y" or "X to Y" (X, Y may be multi-word e.g. LAEM CHABANG, NHAVA SHEVA)
        m = re.search(r"from\s+(.+?)\s+to\s+(\S+(?:\s+\S+)*?)(?:\s+in\s+|\s*$|,|\.)", q, re.IGNORECASE | re.DOTALL)
        if m:
            origin = m.group(1).strip()
            dest = m.group(2).strip()
            if origin and dest and len(origin) < 80 and len(dest) < 80:
                return origin, dest
        m = re.search(r"(\w+(?:\s+\w+)?)\s+to\s+(\w+(?:\s+\w+)?)(?:\s|$|,|\.)", q, re.IGNORECASE)
        if m:
            origin = m.group(1).strip()
            dest = m.group(2).strip()
            if origin and dest:
    ...
    async def _fetch_structured_routes_for_query(
        self, organization_id: int, query: Optional[str]
    ) -> List[Dict[str, Any]]:
        """If query mentions origin/destination, query PostgreSQL routes and return them for answer context."""
        origin, dest = self._parse_origin_destination_from_query(query)
        if not origin or not dest:
            return []
        print(f"[RATE SHEET SEARCH] Parsed origin/dest from query: {origin!r} → {dest!r}")
        from app.core.database import AsyncSessionLocal
        from app.services.structured_data_service import StructuredDataService
        # Try exact parsed ports first, then common spelling variants
        origin_variants = [origin]
        dest_variants = [dest]
        o_upper = origin.upper()
        d_upper = dest.upper()
        if "BANGKOK" in o_upper or "BANGKOK" in origin:
            origin_variants.extend(["BANGKOK", "LAEM CHABANG"])
        if "LAEM" in o_upper or "CHABANG" in o_upper:
            origin_variants.extend(["LAEM CHABANG", "BANGKOK"])
        if "KOLKATTA" in d_upper or "KOLKATA" in d_upper or "KOLKATTA" in dest:
            dest_variants.extend(["KOLKATTA", "KOLKATA"])
        if "KOLKATA" in d_upper:
            dest_variants.extend(["KOLKATA", "KOLKATTA"])
        async with AsyncSessionLocal() as session:
            service = StructuredDataService()
            for o in origin_variants:
                for d in dest_variants:
                    ...
                    if routes:
                        print(f"[RATE SHEET SEARCH] Structured routes found: {len(routes)} for {o} → {d}")
                        return routes
        print(f"[RATE SHEET SEARCH] No structured routes in DB for parsed origin/dest: {origin!r} → {dest!r}")
        return []
    ...
    async def _get_intent_for_query(self, query: Optional[str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=8.0) as client:
                ...
                print(f"[INTENT] HTTP {resp.status_code} body={body}")
                logger.warning(f"Intent classifier returned {resp.status_code}: {body}")
        except Exception as e:
            err_msg = f"{type(e).__name__}: {e}" if str(e) else type(e).__name__
            print(f"[INTENT] Error: {err_msg}")
            logger.warning(f"Intent classifier unavailable or error: {err_msg}", exc_info=True)
        return None
    ...
    async def search_rate_sheets(
        ...
        try:
            ...
            # Normalize query for port spellings (e.g. Kolkata → KOLKATA, KOLKATTA) so vector search and answer match rate sheets
            vector_search_query = search_query
            if query and query.strip():
                vector_search_query = (await self.rerank_service.normalize_search_query(query.strip())) or search_query
                if vector_search_query != search_query:
                    print(f"[RATE SHEET SEARCH] Using normalized query for vector search (port spellings expanded)")
            ...
            if self._is_simple_query(query, origin_code, destination_code, container_type):
                ...
                match_query_lower = vector_search_query.lower() if vector_search_query else query_lower
                formatted_fast = []
                for result in vector_results:
                    metadata = result.get("metadata", {})
                    document = result.get("document", "")
                    ...
                    formatted_fast.append({
                        ...
                        "document": document,
                        "document_preview": document[:1000],
                        "matching_data": self._extract_matching_data(document, match_query_lower)
                    })
                ...
                # Try to get exact rates from PostgreSQL for "from X to Y" queries so we don't rely only on document text
                structured_routes = await self._fetch_structured_routes_for_query(organization_id, query)
                ai_answer_fast = await self.rerank_service.generate_answer(
                    query=search_query, results=top_fast, answer_style=fast_style, intent_result=intent_fast, structured_routes=structured_routes
                )
            ...
            if self._should_skip_orchestrator(query, origin_code, destination_code, container_type):
                ...
                structured_routes_v = await self._fetch_structured_routes_for_query(organization_id, query)
                ai_answer_v = await self.rerank_service.generate_answer(
                    query=search_query, results=top_v, answer_style=vector_style, intent_result=intent_v, structured_routes=structured_routes_v
                )
            ...
            if not formatted_results:
                ...
                fallback_vector_query = vector_search_query
                if carrier_name:
                    fallback_vector_query += f" carrier {carrier_name}"
                if origin_code:
                    fallback_vector_query += f" origin {origin_code}"
                if destination_code:
                    fallback_vector_query += f" destination {destination_code}"
                if container_type:
                    fallback_vector_query += f" container {container_type}"
                filters = {"carrier_name": carrier_name} if carrier_name else {}

                logger.info(f"Stage 1 (fallback): Vector search for query: '{fallback_vector_query}' (limit=10 for latency)")
                vector_results = await self.embedding_service.search_rate_sheets(
                    query=fallback_vector_query,
                    organization_id=organization_id,
                    limit=10,
                    filters=filters
                )
