...
@router.get("/{rate_sheet_id}/data")
async def get_rate_sheet_data(
    rate_sheet_id: str,
    organization_id: int = Query(...)
):
    """
    Get full structured data for a rate sheet (metadata + routes + pricing_tiers + surcharges).
    Data is stored in PostgreSQL. If no structured data exists, returns minimal data from ChromaDB
    (same shape with empty routes/pricing_tiers/surcharges and status='no_structured_data').
    """
    ...
    async with AsyncSessionLocal() as session:
        ...
        data = await service.get_structured_data_full(
            session=session,
            rate_sheet_id=rate_sheet_id,
            organization_id=organization_id
        )
    ...
    # No structured data in PostgreSQL - try ChromaDB so we can show at least metadata
    rate_sheet_svc = RateSheetService()
    rate_sheet = await rate_sheet_svc.get_rate_sheet(
        rate_sheet_id=rate_sheet_id,
        organization_id=organization_id
    )
    # Fallback: get-by-id can 404 in some setups (e.g. vector DB reachability). Use same search as list and find by id.
    if not rate_sheet:
        search_results = await rate_sheet_svc.embedding_service.search_rate_sheets(
            query="rate sheet",
            organization_id=organization_id,
            limit=1000,
        )
        for r in search_results:
            if r.get("id") == rate_sheet_id:
                rate_sheet = {
                    "id": r.get("id"),
    ...
    if not rate_sheet:
        raise HTTPException(
            status_code=404,
            detail="Rate sheet not found. Use the exact id from the list and the same organization_id (e.g. ?organization_id=1)."
        )
    meta = rate_sheet.get("metadata") or {}
    return {
        "rate_sheet_id": rate_sheet_id,
        "organization_id": int(meta.get("organization_id", organization_id)),
...
@router.patch("/{rate_sheet_id}")
async def update_rate_sheet_metadata(
    request: Request,
    rate_sheet_id: str,
    organization_id: int = Query(...),
):
    """
    Update rate sheet metadata: title, carrier_name, valid_from, valid_to.
    Body: { "title?", "carrier_name?", "valid_from?", "valid_to?" } (all optional).
    Dates in ISO format (e.g. "2026-01-01", "2026-12-31T23:59:59Z").
    """
    ...
    valid_to_str = body.get("valid_to")
    ...
    if valid_from_str:
        try:
            ...
            raise HTTPException(status_code=400, detail="invalid valid_from date format")
    if valid_to_str:
        try:
            valid_to = datetime.fromisoformat(valid_to_str.replace("Z", "+00:00"))
        except Exception:
            raise HTTPException(status_code=400, detail="invalid valid_to date format")
    ...
    if not record:
        raise HTTPException(status_code=404, detail="Rate sheet not found")
    return {
        "rate_sheet_id": record.rate_sheet_id,
        "title": record.title,
...
@router.post("/process-inbound-reply", status_code=200)
async def process_inbound_reply(request: Request):
    """
    ...
    Body: user_id, organization_id, email_content, subject (optional), from_email (optional), thread_id (optional).
    """
    try:
        body = await request.json()
        ...
        logger.error(f"Error in process-inbound-reply: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
