...
@app.api_route("/api/auth/microsoft", methods=["GET", "OPTIONS"])
async def proxy_auth_microsoft_initiate(request: Request):
    """Proxy Microsoft OAuth initiate to authentication service"""
    url = f"{settings.AUTHENTICATION_SERVICE_URL}/api/auth/microsoft"
    return await proxy_request(request, url, "Authentication Service", follow_redirects=False)


@app.api_route("/api/auth/microsoft/callback", methods=["GET", "OPTIONS"])
async def proxy_auth_microsoft_callback(request: Request):
    """Proxy Microsoft OAuth callback to authentication service"""
    url = f"{settings.AUTHENTICATION_SERVICE_URL}/api/auth/microsoft/callback"
    return await proxy_request(request, url, "Authentication Service", follow_redirects=False)


@app.api_route("/api/public/{path:path}", methods=["GET", "POST", "OPTIONS"])
async def proxy_public(request: Request, path: str):
    """Proxy public endpoints (no auth) e.g. GET /api/public/organizations/by-slug to user service"""
    url = f"{settings.USER_SERVICE_URL}/api/public/{path}"
    return await proxy_request(request, url, "User Service (public)", default_timeout=30.0)
...
async def proxy_orders_root(request: Request):
    """Proxy requests to Order service root"""
    url = f"{settings.ORDER_SERVICE_URL}/api/orders"
    return await proxy_request(request, url, "Order Service")
...
async def proxy_orders(request: Request, path: str):
    ...
    url = f"{settings.ORDER_SERVICE_URL}/api/orders/{path}"
    return await proxy_request(request, url, "Order Service")


@app.api_route("/api/customers", methods=["GET", "POST", "OPTIONS"])
async def proxy_customers_root(request: Request):
    """Proxy requests to Customer service root"""
    url = f"{settings.CUSTOMER_SERVICE_URL}/api/customers"
    return await proxy_request(request, url, "Customer Service")


@app.api_route("/api/customers/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"])
async def proxy_customers(request: Request, path: str):
    """Proxy requests to Customer service"""
    url = f"{settings.CUSTOMER_SERVICE_URL}/api/customers/{path}"
    return await proxy_request(request, url, "Customer Service")
