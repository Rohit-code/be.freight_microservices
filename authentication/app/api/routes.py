...
@router.post("/signup", response_model=SignupVerificationSentResponse)
...
async def send_verification_email(request: Request):
    """Resend OTP to email (e.g. after signup). Body: { \"email\": \"...\" }."""
    body = await request.json()
    email = (body.get("email") or "").strip()
    if not email:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="email required")
    return await auth_service.send_verification_email(email)


@router.post("/verify-email", response_model=AuthResponse)
async def verify_email(payload: VerifyEmailRequest):
    """Verify OTP and return JWT (logs user in)."""
...
def microsoft_oauth_initiate(request: Request):
    """Initiate Microsoft OAuth flow - redirects to Azure AD"""
    return auth_service.initiate_microsoft_oauth(request)


@router.get("/microsoft/callback")
async def microsoft_oauth_callback(request: Request):
    """Handle Microsoft OAuth callback - receives code and exchanges for token"""
    return await auth_service.handle_microsoft_callback(request)
