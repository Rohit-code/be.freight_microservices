...
class Settings(BaseSettings):
    ...
    microsoft_client_secret: str = Field(
        default="",
        validation_alias=AliasChoices("MICROSOFT_CLIENT_SECRET", "microsoft_client_secret")
    )
    ...
    microsoft_redirect_uri: str = Field(
        ...
        validation_alias=AliasChoices("MICROSOFT_REDIRECT_URI", "microsoft_redirect_uri"),
    )

    @property
    def effective_microsoft_redirect_uri(self) -> str:
        """Effective Microsoft OAuth redirect URI."""
    ...
    # Resend (OTP / verification emails). Key must be in env only; never commit.
...
if not settings.microsoft_client_id:
    settings.microsoft_client_id = os.getenv("MICROSOFT_CLIENT_ID", "")
...
if not settings.microsoft_tenant_id:
    settings.microsoft_tenant_id = os.getenv("MICROSOFT_TENANT_ID", "")
if settings.microsoft_redirect_uri == "http://localhost:8000/api/auth/microsoft/callback":
    ms_redirect = os.getenv("MICROSOFT_REDIRECT_URI")
    if ms_redirect:
        settings.microsoft_redirect_uri = ms_redirect
