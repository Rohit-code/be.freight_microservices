...
from ..utils.resend_sender import send_email as resend_send_email, build_otp_email_html
import secrets
import logging
...
class AuthService:
    ...
    async def login(self, payload: LoginRequest) -> AuthResponse:
        ...
        async with AsyncSessionLocal() as session:
            ...
            token = generate_jwt_token(user.id, user.email, is_staff=user.is_staff, is_superuser=user.is_superuser)
            ...
            user_out = UserOut(
                ...
                has_microsoft_connected=bool(user.microsoft_access_token),
            )
            return AuthResponse(user=user_out, token=token)

    async def signup(self, payload: SignupRequest) -> SignupVerificationSentResponse:
        """Create user (email_verified_at=None), send OTP via Resend, return verification-sent (no JWT until verify-email)."""
        email = payload.email.lower().strip()
        ...
        async with AsyncSessionLocal() as session:
            ...
            # Generate OTP, store, send email (no JWT yet)
            code = "".join(random.choices(string.digits, k=6))
            expires_at = datetime.now(timezone.utc) + timedelta(minutes=15)
            otp_row = EmailVerificationCode(
                email=email,
                code=code,
            ...
            ok, err = resend_send_email(
                to=email,
                subject="Verify your email",
                html=build_otp_email_html(code, expiry_minutes=15),
            )
            if not ok:
                logger.warning("Failed to send verification email: %s", err)
                # Still return success so user can request resend
            return SignupVerificationSentResponse(message="Verification email sent", email=email)

    async def send_verification_email(self, email: str) -> dict:
        """Send a new OTP to the given email. User must exist and be unverified (or allow resend)."""
        ...
        async with AsyncSessionLocal() as session:
            ...
            code = "".join(random.choices(string.digits, k=6))
            expires_at = datetime.now(timezone.utc) + timedelta(minutes=15)
            await session.execute(delete(EmailVerificationCode).where(EmailVerificationCode.email == email))
            otp_row = EmailVerificationCode(email=email, code=code, expires_at=expires_at)
            session.add(otp_row)
            await session.commit()
            ...
            if not ok:
                raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=err or "Failed to send email")
            return {"message": "Verification email sent", "email": email}

    async def verify_email(self, payload: VerifyEmailRequest) -> AuthResponse:
        """Verify OTP and mark user email_verified_at; return JWT so user is logged in."""
        email = payload.email.lower().strip()
        ...
        async with AsyncSessionLocal() as session:
            ...
            await session.execute(delete(EmailVerificationCode).where(EmailVerificationCode.id == otp_row.id))
    ...
    def initiate_microsoft_oauth(self, request: Request) -> RedirectResponse:
        """Initiate Microsoft OAuth flow - redirects to Azure AD."""
        frontend_url = "http://localhost:3000/api/auth/microsoft/callback"
        try:
            if not settings.microsoft_client_id or not settings.microsoft_client_secret:
                return RedirectResponse(
                    url=f"{frontend_url}?error=oauth_not_configured&details=Microsoft_OAuth_not_configured",
                    status_code=302,
                )
            state = secrets.token_urlsafe(32)
            from ..utils.microsoft_oauth import get_microsoft_authorization_url
    ...
    async def handle_microsoft_callback(self, request: Request) -> RedirectResponse:
        """Handle Microsoft OAuth callback - exchange code and get/create user."""
        code = request.query_params.get("code")
        state = request.query_params.get("state")
        error = request.query_params.get("error")
        frontend_url = "http://localhost:3000/api/auth/microsoft/callback"
        ...
        try:
            from ..utils.microsoft_oauth import exchange_microsoft_code_for_token
            ms_user_info = await exchange_microsoft_code_for_token(code)
            async with AsyncSessionLocal() as session:
                result = await session.execute(
                    select(User).where(User.email == ms_user_info["email"])
                )
                user = result.scalar_one_or_none()
                if user:
                    if not user.microsoft_id:
                        user.microsoft_id = ms_user_info["microsoft_id"]
                        user.is_microsoft_user = True
                    if ms_user_info.get("picture") and not user.picture:
                        user.picture = ms_user_info["picture"]
                    if ms_user_info.get("access_token"):
                        user.microsoft_access_token = ms_user_info["access_token"]
                    if ms_user_info.get("refresh_token"):
                        user.microsoft_refresh_token = ms_user_info["refresh_token"]
                    if ms_user_info.get("token_expiry"):
                        user.microsoft_token_expiry = ms_user_info["token_expiry"]
                else:
                    name_parts = (ms_user_info.get("name") or "").split()
                    first_name = name_parts[0] if name_parts else ""
                    last_name = " ".join(name_parts[1:]) if len(name_parts) > 1 else ""
                    username = ms_user_info["email"].split("@")[0]
                    user = User(
                        email=ms_user_info["email"],
                        username=username,
                        first_name=first_name or None,
                        last_name=last_name or None,
                        picture=ms_user_info.get("picture"),
                        microsoft_id=ms_user_info["microsoft_id"],
                        is_microsoft_user=True,
                        microsoft_access_token=ms_user_info.get("access_token") or "",
                        microsoft_refresh_token=ms_user_info.get("refresh_token") or "",
                        microsoft_token_expiry=ms_user_info.get("token_expiry"),
                    )
                    session.add(user)
                await session.commit()
                await session.refresh(user)
    ...
    async def get_current_user(self, token: str) -> AuthResponse:
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                # Resolve organization_id from user service (for rate sheets, email drafts)
                organization_id = None
                try:
                    import httpx
                    org_url = f"{settings.USER_SERVICE_URL}/api/user/internal/user/{user.id}/organization-id"
                    ...
                    logger.debug(f"Could not get organization_id for user {user.id}: {e}")
    ...
    async def get_admin_dashboard(self, token: str) -> AdminDashboardResponse:
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                microsoft_connected_users = await session.scalar(
                    select(func.count()).select_from(User).where(User.microsoft_access_token.isnot(None))
                )
                ...
                return AdminDashboardResponse(
                    ...
                    microsoft_connected_users=microsoft_connected_users or 0,
