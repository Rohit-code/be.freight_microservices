...
│           /api/constants/*, /api/rate-sheets/*, /api/orders/*, /api/customers/*   │
└───┬─────┬─────┬─────┬─────┬─────┬─────┬──────────────────────────────────────────┘
    │     │     │     │     │     │     │
    ▼     ▼     ▼     ▼     ▼     ▼     ▼
┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐  ┌────────────────────────────┐
...
**Note:** Knowledge Graph (8011), Intent Classifier (8012), Orchestrator (8013), and Decision Engine (8014) are **not** exposed as separate routes on the API Gateway. They are used **internally** by the Rate Sheet Service (e.g. for drafting email responses). Email intake is via **Gmail** (watch + webhook + fetch); **Outlook/Microsoft Graph** mail integration is not yet implemented (Microsoft OAuth is for login only).
...
|----------------------|------|----------------|
| **API Gateway**      | 8000 | Single entry; CORS; proxy to all services; root handler can forward Pub/Sub webhooks to `/api/auth/gmail/webhook`. Explicit proxy for `/api/auth/microsoft` and `/api/auth/microsoft/callback`. |
| **Authentication**   | 8001 | Login/signup, **Google OAuth** and **Microsoft OAuth** (Azure AD), JWT, Gmail/Drive/Sheets/Docs tokens, Gmail watch & webhook, admin CRUD, internal Gmail/list by user. Send email currently via Gmail API only. |
...
| **Order Service**    | 8015 | Orders and tracking for customer portal; JWT-scoped list/get/create and tracking events. |
| **Customer Service** | 8016 | Customer (shipper/BCO) profiles: company, contact, default ports, preferences; scoped by JWT user_id. |
...
**Database-per-service rule:** Each service has its own database on the shared Postgres instance. No service may connect to another service's database. Cross-service data is shared only by IDs (e.g. `user_id`, `customer_id`, `organization_id`); services communicate via HTTP (gateway or service-to-service). This keeps services loosely coupled.
...
- **order_service_db** – Order service (orders, order_tracking_events)
- **customer_service_db** – Customer service (customers: shipper/BCO profiles)
...
| microsoft_id             | String(255)    | Unique, nullable (Azure AD sub) |
| is_microsoft_user        | Boolean        | Default False |
| microsoft_access_token   | Text           | Nullable |
| microsoft_refresh_token  | Text           | Nullable |
| microsoft_token_expiry   | DateTime(TZ)   | Nullable |
...
#### 5.1.6 Microsoft OAuth – initiate

| Step | Actor | Action |
|------|--------|--------|
| 1 | Frontend | User clicks "Continue with Microsoft"; redirects to `GET /api/auth/microsoft` |
| 2 | API Gateway | Proxies to Authentication (explicit route for `/api/auth/microsoft`) |
| 3 | Authentication | Builds Azure AD v2 authorize URL (client_id, tenant_id, redirect_uri, scopes: openid profile email User.Read Mail.Read Mail.Send Files.Read); sets `oauth_state` cookie; returns **RedirectResponse** to Microsoft login |

#### 5.1.7 Microsoft OAuth – callback

| Step | Actor | Action |
|------|--------|--------|
| 1 | Microsoft | After user consents, redirects to `GET /api/auth/microsoft/callback?code=...&state=...` |
| 2 | API Gateway | Proxies to Authentication |
| 3 | Authentication | Verifies `state` cookie; exchanges `code` for tokens at Azure AD token endpoint; GETs user from Microsoft Graph `/me`; gets or creates user in **PostgreSQL users**; sets `microsoft_id`, `is_microsoft_user`, `microsoft_access_token`, `microsoft_refresh_token`, `microsoft_token_expiry` |
| 4 | Authentication | Generates JWT; redirects to frontend callback URL with token (e.g. `http://localhost:3000/api/auth/microsoft/callback?token=...&success=true`) |
...
| /api/orders, /api/orders/*           | Order Service      | 8015 |
| /api/customers, /api/customers/*     | Customer Service   | 8016 |

The gateway defines **explicit** proxy routes for `GET /api/auth/microsoft` and `GET /api/auth/microsoft/callback` (in addition to the catch-all `/api/auth/{path:path}`).
...
- **Environment**  
  Central `.env` in `microservices/`; services read DB URLs, service URLs, JWT secret, Google OAuth, **Microsoft OAuth** (MICROSOFT_CLIENT_ID, MICROSOFT_TENANT_ID, MICROSOFT_CLIENT_SECRET, MICROSOFT_REDIRECT_URI), Gmail Pub/Sub topic, webhook URL, OpenAI key, etc.
...
```
microservices/
├── api_gateway/          # Proxy and CORS (incl. /api/auth/microsoft)
├── authentication/       # Auth, Google + Microsoft OAuth, Gmail webhook, admin
├── constants/            # Static constants/FAQs
├── ai_service/           # OpenAI integration
├── vector_db/            # ChromaDB-like + BGE (pickle)
├── email_service/        # Email CRUD + Vector DB + draft trigger (Gmail)
...
├── decision_engine/
├── order_service/        # Orders and tracking (customer portal)
├── customer_service/    # Customer (shipper/BCO) profiles
...
## 10. Target SaaS Operational Flow (Phases 1–11)

This section describes the **target end-to-end flow** for the freight forwarding SaaS. Each phase is mapped to **current implementation status** in the codebase.

### 10.1 Phase overview and implementation status

| Phase | Name | Current implementation |
|-------|------|------------------------|
| **1** | Customer inquiry intake | **Partial.** Email listener: Gmail watch + webhook + store in Vector DB; AI draft (Intent → Orchestrator → Decision Engine → AI). No dedicated "Email Parser" entity extraction; no CUSTOMER/INQUIRY records or "Inquiry Received" status. Customer Service exists (profiles); Order Service exists (orders, tracking). |
| **2** | Quote creation & sending | **Partial.** Rate Sheet Service: pricing from rate cards (PG + graph + vector), AI draft, send via Gmail. No QUOTE entity, Quote PDF, or Approve/Reject buttons in email. |
| **3** | Customer approval | **Planned.** No "Approve" flow, secure order link, or account-creation email from quote. Customer portal login exists (Customer Service + JWT). |
| **4** | Order submission + payment | **Partial.** Order form and ORDER creation via API; tracking events. No payment gateway integration or "Order Confirmed" status flow. |
| **5** | Shipment initialization | **Planned.** No shipment/route/container/invoice entities or "Awaiting Carrier Allocation" status. |
| **6** | Carrier allocation | **Planned.** No ops admin flow for carrier/vessel/voyage or "Booking Confirmed" status. |
| **7** | Document automation | **Planned.** No document engine, MBL/HBL/Commercial Invoice/Shipping Instruction/Packing List, or S3/storage. |
| **8** | Shipment execution & tracking | **Partial.** Order Service has tracking events; admin can update milestones. No event broadcast or customer-portal auto-update specified. |
| **9** | Customer portal experience | **Partial.** Customer login, dashboard, tracking, documents (as implemented in frontend). Support request not specified. |
| **10** | Notifications engine | **Planned.** No dedicated notification service for email/status alerts/payment/shipment milestones. |
| **11** | Completion & closure | **Planned.** No "Completed" status, final invoice, payment verification, or archive flow. |

### 10.2 Phase flows (target)

**PHASE 1 — Customer inquiry intake**

- Customer sends email inquiry → Email listener triggered → Fetch email (Outlook/Gmail API) → AI Email Parser extracts customer name, POL, POD, cargo, container type, weight/volume, dates → Validate → Create CUSTOMER (if new) → Create INQUIRY → Status = Inquiry Received.

**PHASE 2 — Quote creation & sending**

- Inquiry data → Pricing engine (rate cards + surcharges, margin rules) → Costing breakdown → Create QUOTE ID → Generate Quote PDF → AI drafts quotation email → Send to customer (Quote PDF, Approve/Reject buttons) → Status = Quote Sent.

**PHASE 3 — Customer approval**

- Customer clicks Approve → Verify quote validity → Generate secure order link → Send order-creation email → Customer opens order portal → Account creation (username/password) → Link account ↔ quote.

**PHASE 4 — Order submission + payment**

- Customer fills order form (shipper, consignee, cargo, documents, terms) → Submit → Payment gateway → On success: convert quote to order, ORDER ID, Status = Order Confirmed.

**PHASE 5 — Shipment initialization**

- Create shipment entities (Shipment ID, Route ID, Container, Invoice) → Preliminary route → Status = Awaiting Carrier Allocation.

**PHASE 6 — Carrier allocation**

- Ops admin selects carrier, vessel, voyage → Confirm space booking → Update route → Status = Booking Confirmed.

**PHASE 7 — Document automation**

- Document engine: auto-generate MBL, HBL, Commercial Invoice, Shipping Instruction, Packing List → Store (e.g. S3) → Send to customer.

**PHASE 8 — Shipment execution & tracking**

- Admin updates milestones (Gate In, Loaded, Vessel Departed, Transshipment, Arrived, Customs Cleared, Out for Delivery) → Tracking event saved → Event broadcast → Customer portal auto-updates.

**PHASE 9 — Customer portal experience**

- Dashboard: active shipments, tracking timeline, documents, invoice status, payment history; actions: download BL, view tracking, raise support request.

**PHASE 10 — Notifications engine (parallel)**

- On events: send email updates, status alerts, payment confirmations, shipment milestones.

**PHASE 11 — Completion & closure**

- Shipment delivered → Final invoice → Payment verified → Status = Completed → Archive shipment.

### 10.3 System-side flows (target)

- Quote expiry checker, payment reconciliation, document backup, email sync listener, audit logging, user activity tracking.

### 10.4 Master flow (summary)

Email inquiry → AI parse → Quote → Approval → Account creation → Order form → Payment → Shipment creation → Carrier allocation → Document generation → Tracking updates → Customer portal → Delivery → Closure.
