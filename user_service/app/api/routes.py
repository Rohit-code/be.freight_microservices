...
# Public endpoints (no auth; e.g. customer portal resolving org by slug)
...
# ========== PUBLIC ENDPOINTS (No auth; customer portal org resolution) ==========

@public_router.get("/organizations/by-slug")
async def get_organization_by_slug_public(slug: str = Query(..., min_length=1)):
    """
    Resolve organization by slug. Returns organization_id and name only.
    Used by customer portal when user enters org URL/slug (Flow B). No auth required.
    """
    organization = await get_organization_by_slug(slug.strip())
    if not organization:
        raise HTTPException(status_code=404, detail="Organization not found")
